...
- **`npm pack` is the cleanest way to stage upstream source.** Packing published packages via `npm pack <pkg>@<version>` then extracting the tarballs gives a deterministic, reproducible snapshot of upstream source without git clones or network-dependent install trees. This should be the default pattern for any scaffold task that ports upstream code.
