...
**Note:** This task intentionally inserts build routing **before** test routing in the if/else chain. Task 7 fixes the ordering to satisfy AC14.
...
Update `src/rtk/bash-filter.ts` — add build routing **before** test routing (deliberately wrong order):
...
Update `src/rtk/bash-filter.ts` to add linter routing at the end of the chain (before fallback):
...
**Step 2 — Run test, verify it fails**
Run: `npx vitest run tests/bash-filter.test.ts`  
Expected: FAIL — `AssertionError: expected "aggregateTestOutput" spy to have been called`
