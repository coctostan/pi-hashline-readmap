...
Create the five fixture files with the following contents.
...
Replace `tests/bash-filter.test.ts` with:
...
**Step 2 — Run test, verify it fails**
Run: `npx vitest run tests/bash-filter.test.ts`  
Expected: FAIL — missing exports (e.g. `isTestCommand is not exported from '../src/rtk/bash-filter.js'`)
...
Update `src/rtk/bash-filter.ts` to route git commands after test routing:
...
**Note:** Linter is checked before build so that `tsc --noEmit` routes to linter (AC9) rather than build.

**Step 1 — Write the failing test**

In `tests/bash-filter.test.ts`, add:

```typescript
import * as linterModule from "../src/rtk/linter.js";
```

and inside `describe("filterBashOutput routing", ...)` add:
...
export function filterBashOutput(command: string, output: string): FilterResult {
  ...
  if (isTestCommand(command)) {
    ...
    result = filterBuildOutput(stripped, command) ?? stripped;
  } else {
    result = stripped;
  }
...
In `tests/bash-filter.test.ts`, add inside `describe("filterBashOutput routing", ...)`:

```typescript
it("test command wins over build when both match (AC14 example: cargo test)", () => {
  // AC14 gives "cargo test" as an example of an overlapping command.
  // This test forces the overlap by asserting isBuildCommand matches it.
  expect(isTestCommand("cargo test")).toBe(true);
  expect(isBuildCommand("cargo test")).toBe(true);
  ...
  expect(testSpy).toHaveBeenCalledWith("some output", "cargo test");
...
**Step 2 — Run test, verify it fails**
Run: `npx vitest run tests/bash-filter.test.ts`  
Expected: FAIL — `AssertionError: expected false to be true` (for `isBuildCommand("cargo test")`)

**Step 3 — Write minimal implementation**

Update `isBuildCommand` in `src/rtk/bash-filter.ts` to also match `cargo test` (creating the overlap used by AC14), while preserving routing priority (test check stays before build check):

```typescript
export function isBuildCommand(command: string): boolean {
  const c = command.toLowerCase();
  return ["tsc", "cargo build", "cargo test", "npm run build"].some((t) => c.includes(t));
}
```
...
Wrap routing logic in a try/catch in `src/rtk/bash-filter.ts`:
...
describe("bash filter integration", () => {
  it("tool_result handler is registered and only modifies bash results", async () => {
    ...
    expect(await handlers["tool_result"](readEvent)).toBeUndefined();
...
Expected: FAIL — `AssertionError: expected undefined to be defined` (tool_result handler not registered yet)
...
Update `tests/bash-filter-integration.test.ts`:

1) Update the existing vitest import at the top of the file to include `vi` (e.g. `import { describe, it, expect, vi } from "vitest";`).
2) Append the following test block to the bottom of the file:
