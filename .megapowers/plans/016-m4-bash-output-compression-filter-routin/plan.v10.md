...
describe("filterBashOutput routing", () => {
  ...
  it("test command wins over build when both match (AC14: cargo test && cargo build)", () => {
    // Use a compound command that naturally matches both predicates
    const cmd = "cargo test && cargo build";
...
```
**Step 2 — Run test, verify it passes (no new production code needed)**
Run: `npx vitest run tests/bash-filter.test.ts`  
Expected: PASS — no production code change needed. The overlap is achieved naturally via a compound command; routing priority (test → git → linter → build) was already implemented in Task 6.

**Step 3 — No production code change**

No implementation needed. The routing priority (test before build) was established in Tasks 3–6. This task only verifies the priority is correct for compound commands that naturally match both predicates.
...
Replace `tests/entry-point.test.ts` with (adds `on() {}` to mockPi so it won't break when `index.ts` starts calling `pi.on(...)`):
...
Replace `tests/bash-filter-integration.test.ts` with:
```typescript
import { describe, it, expect, vi } from "vitest";
import { fileURLToPath, pathToFileURL } from "node:url";
import { dirname, resolve } from "node:path";
...
| 2 | 2, 3, 4 | ANSI stripping (Task 2: fallback; Tasks 3–4: spy asserts technique receives stripped input) |
| 3 | 2 | Empty input |
