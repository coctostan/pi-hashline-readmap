# Plan: Symbol-Addressable Read (M2) — Revised v4
...
2) In `execute`, after `const total = allLines.length;`, replace current start/end setup with:
```ts
...
3) Keep using existing formatting block, but in the line formatter make symbol reads relative-numbered for now:
```ts
const formatted = selected
  .map((line, i) => {
    const num = params.symbol ? i + 1 : startLine + i;
...
### Task 16: Prepend symbol header on found reads (large file path) [depends: 15]
**Covers:** AC 14, AC 19 (large)
...
Append:
```ts
it("prepends symbol header for large-file symbol reads", async () => {
  const result = await callReadTool({
...
**Step 2 — Run test, verify it fails**
Run: `npx vitest run tests/symbol-read-integration.test.ts -t "prepends symbol header for large-file symbol reads"`
Expected: FAIL — symbol output currently has no `[Symbol: ...]` header.

**Step 3 — Write minimal implementation**
In `src/read.ts`, before final return, prepend when symbol is found:
```ts
...
it("returns disambiguation text and no hashlines for ambiguous symbol query", async () => {
  ...
  expect(text).toContain("initialize (method)");
...
**Step 3 — Write minimal implementation**
Inside symbol branch in `src/read.ts`, after `const lookup = findSymbol(...)`:
```ts
if (lookup.type === "ambiguous") {
  ...
  return {
    content: [{
      type: "text",
      text: [`Symbol '${params.symbol}' is ambiguous.`, "Matches:", ...lines, "Use dot notation to disambiguate."].join("\n"),
    }],
    details: {},
  };
}
...
**Step 3 — Write minimal implementation**
In `src/read.ts`:
1) Add accumulator after conflict validation:
```ts
let symbolWarning: string | undefined;
```
2) In symbol lookup branch, add not-found warning:
```ts
...
```
3) Before final return, prepend warning:
```ts
if (symbolWarning) text = symbolWarning + text;
```
...
**Step 3 — Write minimal implementation**
In symbol branch in `src/read.ts`, after `const fileMap = await getOrGenerateMap(absolutePath);`:
```ts
if (!fileMap) {
...
### Task 22: Symbol-read anchors are valid for edit tool [depends: 21]
**Covers:** AC 20 (and completes AC 13 line anchoring)
...
Append:
```ts
it("produces symbol-read anchors that apply at original file line numbers", async () => {
  const result = await callReadTool({
    path: resolve(fixturesDir, "small.ts"),
    symbol: "createDemoDirectory",
  });
...
**Step 2 — Run test, verify it fails**
Run: `npx vitest run tests/symbol-read-integration.test.ts -t "produces symbol-read anchors that apply at original file line numbers"`
Expected: FAIL — symbol rows are currently numbered relative to 1..N, so `rows[0].line` is `1` and edit lands at wrong line.

**Step 3 — Write minimal implementation**
In `src/read.ts` formatter block, replace:
```ts
const num = params.symbol ? i + 1 : startLine + i;
```
with:
...
### Task 23: Document symbol parameter in read prompt [no-test] [depends: 22]
**Covers:** AC 21
...
## Notes
- Tasks are strictly one test + one implementation change.
- Ambiguity handling is explicitly covered at exact, nested, case-insensitive, and partial tiers.
- AC 7 and AC 9 now have explicit tests (not implicit-only coverage).
