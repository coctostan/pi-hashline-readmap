# Plan: Symbol-Addressable Read (M2) — Revised v7
...
**Step 3 — Write minimal implementation**
At top of `findSymbol(...)`, normalize query with `.trim()` and use the trimmed value for all matching tiers.

Replace the entire `findSymbol(...)` function with:
```ts
export function findSymbol(map: FileMap, query: string): SymbolLookupResult {
  const q = query.trim();
...
Expected: FAIL — current output is a full-file read (more than 5 hashline rows).
...
2) Replace the existing `startLine` / `endIdx` / `selected` block:
```ts
...
```
with this block (note: this intentionally slices the symbol body but does **not** yet adjust the printed line numbers; that comes in Task 16):
```ts
...
### Task 16: Symbol-read anchors use original file line numbers (edit-compatible) [depends: 15]
**Covers:** AC 20
...
**Step 2 — Run test, verify it fails**
Run: `npx vitest run tests/symbol-read-integration.test.ts -t "symbol read anchors refer to original file line numbers (edit-compatible)"`
Expected: FAIL — output rows are numbered `1-5` (not `45-49`), so `rows[0].line` is `1`.

**Step 3 — Write minimal implementation**
Modify `src/read.ts`:

Replace the Task 15 block that computes `startLine`, `endIdx`, and `selected` with this block:
```ts
let startLine = params.offset ? Math.max(1, params.offset) : 1;
let endIdx = params.limit ? Math.min(startLine - 1 + params.limit, total) : total;
...
### Task 17: Prepend symbol header on found reads [depends: 16]
**Covers:** AC 14, AC 19
...
**Step 3 — Write minimal implementation**
In `src/read.ts`, replace the block that starts at `const truncation = truncateHead(...)` and runs through the final `return { ... }` with:
...
// New: symbol header for found symbol reads
...
### Task 18: Suppress structural map for found symbol reads [depends: 17]
**Covers:** AC 15
...
Expected: FAIL — current truncated-path logic still appends the structural `File Map:` section.

**Step 3 — Write minimal implementation**
In `src/read.ts`, change the map-append condition from:
```ts
if (truncation.truncated && !params.offset && !params.limit) {
```
to:
...
### Task 19: Ambiguous symbol query returns disambiguation text only [depends: 18]
**Covers:** AC 16
...
Expected: FAIL — current behavior returns hashlined file content instead of a disambiguation-only message.

**Step 3 — Write minimal implementation**
In `src/read.ts`, replace the entire `if (params.symbol) { ... }` block (the one that currently only handles `lookup.type === "found"`) with:
...
### Task 20: Not-found warning + fallback normal read [depends: 19]
**Covers:** AC 17
...
Expected: FAIL — the tool falls back to a normal read but does not prepend any warning.

**Step 3 — Write minimal implementation**
In `src/read.ts`:

1) After the `symbol + offset/limit` conflict validation and before the `if (params.symbol) { ... }` block, add:
```ts
let symbolWarning: string | undefined;
```

2) Replace the entire `if (params.symbol) { ... }` block with this version (adds **not-found** warnings while preserving **ambiguous** and **found** behaviors):
```ts
if (params.symbol) {
  const fileMap = await getOrGenerateMap(absolutePath);
  if (fileMap) {
...
3) In the truncation/return block, just before the final `return`, prepend the warning when present:
```ts
if (symbolWarning) {
...
### Task 21: Cap not-found symbol list at 20 [depends: 20]
**Covers:** AC 17
...
Expected: FAIL — warning currently includes all 25 symbol names.

**Step 3 — Write minimal implementation**
In the not-found warning code in `src/read.ts`, change the list build from:
```ts
const available = fileMap.symbols.map((s) => s.name).join(", ");
```
to:
...
### Task 22: Unmappable-file warning fallback [depends: 21]
**Covers:** AC 18
...
**Step 3 — Write minimal implementation**
In `src/read.ts`, replace the entire `if (params.symbol) { ... }` block with this complete version (adds the **unmappable warning** while preserving **ambiguous**, **not-found**, and **found** behaviors):
...
### Task 23: Verify symbol reads behave correctly on large/truncated files [no-test] [depends: 18]

**Justification:** Verification-only. AC19 (large-file symbol reads) is already exercised by Task 18’s integration test (large fixture + truncation path). This task just calls out the explicit verification run.
...
### Task 24: Document symbol parameter in read prompt [no-test] [depends: 22]
**Covers:** AC 21
...
Replace `prompts/read.md` contents with:
```md
