...
- `unclipped: true`, available only with the `read.allowUnclipped: true` setting at tool registration, returns complete selected source text and bundled support without line, byte, or per-line display caps. Existing selections still apply. Omission or `false` keeps normal caps. This option has no replacement ceiling and can exhaust model context. Without opt-in, the parameter is absent from the schema and direct calls that supply it fail. See [configuration](../docs/configuration.md#unclipped-reads).
